...
def Push_high():    
    ...
    time.sleep(0.5)
    motor.dxlSetVelo([23, 72, 13], [2, 3, 4])                    # set final velocity (new)
    motor.simMotorRun([87, 218, 123],[2, 3, 4])                   # move arm to chamber (new)
    Open()                                                        # let go
...
def Pull_high():    
    ...
    Close()                                                       # grab battery
    print("remove chamber")
    motor.dxlSetVelo([23, 82, 33], [2, 3, 4])                    # set pull out velocity (new)
