...
global DXL_IDs # Set the motor ID for each dynamixel. ID 0,1,2 is base/bicep/forearm motors
DXL_IDs = [0,1,2,3,4]
global motorNum
...
def dxlPresPos(DXL_IDs: list[int])->list[int]:
    ...
    print("DXL IDs being read: ", DXL_IDs)
    for id in range(idNum): #Reads the current position of the motor
        dxl_present_position[id] = ReadMotorData(DXL_IDs[id], ADDR_PRESENT_POSITION)
    print("Present positions are: ", dxl_present_position)
    return (dxl_present_position)

def dxlPresAngle(DXL_IDs):
    idNum = len(DXL_IDs)
    dxl_present_position = [0] * idNum
...
def dxlSetVelo(vel_array, DXL_IDs):
    if (len(vel_array) == len(DXL_IDs)):
        idNum = len(DXL_IDs)
        for id in range(idNum):
                    WriteMotorData(DXL_IDs[id], ADDR_PROFILE_VELOCITY, vel_array[id])
    else:
...
def dxlGetVelo(DXL_IDs):
    idNum = len(DXL_IDs)
    dxl_present_velocity = [0] * idNum

    print("DXL IDs being read: ", DXL_IDs)
    for id in range(idNum):
        dxl_present_velocity[id] = ReadMotorData(DXL_IDs[id], ADDR_PROFILE_VELOCITY)
    print("Velocities are ", dxl_present_velocity)
    print("-------------------------------------")
    return (dxl_present_velocity)


def motorRunWithInputs(angle_inputs, DXL_IDs):
    idNum = len(DXL_IDs)
    ...
    if (len(angle_inputs) == idNum):
        ...
        print("Motors are rotating. DXL ID: ", DXL_IDs)
        ...
        while 1:
            ...
            for id in range(idNum):
                WriteMotorData(DXL_IDs[id], ADDR_GOAL_POSITION, dxl_goal_inputs[id])

                #Read position for each motor and set status of motor
                dxl_end_position[id], movementStatus[id] = motor_check(DXL_IDs[id], dxl_goal_inputs[id]) #Read position for the motor
            ...
            for id in range(idNum):
                print("Angle for Dynamixels %03d after execution is %03d ----------------------------" % (DXL_IDs[id], dxl_end_angle[id]))
            # ------------------------------------------------------------------------------------------------------------------------------------------------------
...
def simMotorRun(angle_inputs, DXL_IDs):
    idNum = len(DXL_IDs)
    ...
    if (len(angle_inputs) == idNum):
        ...
        print("Motors are simultaneously rotating. DXL ID: ", DXL_IDs)
        # ------------------Start to execute motor rotation------------------------
        while 1:
            #Convert angle inputs into step units for movement
            for id in range(idNum):
            ...
            simWrite(dxl_goal_inputs, DXL_IDs)
            dxl_end_position, movementStatus = simPosCheck(dxl_goal_inputs, DXL_IDs)
            for id in range(idNum):
                dxl_end_angle[id] = _map(dxl_end_position[id], 0, 4095, 0, 360)
            
            # for id in range(idNum):
            #     print("Angle for Dynamixel:%03d is %03d ----------------------------" % (DXL_IDs[id], dxl_end_angle[id]))
            # ------------------------------------------------------------------------------------------------------------------------------------------------------
            print("-------------------------------------")
            return movementStatus
    else:
...
def motorRun(angle_inputs, DXL_IDs):
    idNum = len(DXL_IDs)
...
def simWrite(dxl_goal_inputs, DXL_IDs):
    idNum = len(DXL_IDs)
    ...
    for id in range(idNum):
        dxl_addparam_result = motor_sync_read.addParam(DXL_IDs[id])
        # if dxl_addparam_result != True:
        #     print("[ID:%03d] groupSyncRead addparam failed" % DXL_IDs[id])
    ...
    #Add goal position input values of bicep and forearm motors to Syncwrite parameter storage
        dxl_addparam_result = motor_sync_write.addParam(DXL_IDs[id], param_goal_position[id])
        # if dxl_addparam_result != True:
        #     print("[ID:%03d] groupSyncWrite addparam failed" % DXL_IDs[id])
...
#The functions take in an array of X angle inputs and an array of X dynamixel IDs. Running the functions will drive the X dynamixels to the desired angle inputs simultaneously
def write(dxl_goal_inputs, DXL_IDs):
    idNum = len(DXL_IDs)
    #Intializate simultaneous motor movement
    global motor_sync_write
    motor_sync_write = GroupSyncWrite(portHandler, packetHandler, ADDR_GOAL_POSITION, LEN_GOAL_POSITION)
...
def simPosCheck(dxl_goal_inputs, DXL_IDs):
    idNum = len(DXL_IDs)
    def simReadData():
        dxl_present_position = [0] * idNum
        ...
        for id in range(idNum):
            dxl_present_position[id] = motor_sync_read.getData(DXL_IDs[id], ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)
            #print("ID:%03d, position = %03d" % (DXL_IDs[motorIndex], dxl_present_position[motorIndex]))

        return dxl_present_position
    
    print("Simultaneous position checking. DXL IDs being read: ", DXL_IDs)
