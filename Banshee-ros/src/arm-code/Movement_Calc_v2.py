...
def angle_Calc(coor, CLAW_MODE):  # coor[] = [Px, Py, Pz]
    if CLAW_MODE == 0:
        ...
        phi = 45  # claw at 45-degree angle
    ...
    # Function to calculate the base angle
    def base_Theta():
        if coor[0] == 0:  # if the base theta is 90 or -90 degree
            theta1 = 90 if coor[1] > 0 else -90
        elif coor[0] < 0:  # if base theta is greater than 90 or less than -90
            theta1 = math.atan(coor[1] / coor[0]) + (180 if coor[1] > 0 else -180)
        else:  # if base theta is within [-90,90]
            theta1 = math.atan(coor[1] / coor[0])  # theta1 = arctan(Py/Px)

        return math.degrees(theta1), theta1

    # Function to calculate the elbow angle
    def elbow_Theta():
        cosTheta3 = (math.pow(r2, 2) + math.pow(z2, 2) - (math.pow(BICEP_DIST, 2) + math.pow(FOREARM_DIST, 2))) / (2 * BICEP_DIST * FOREARM_DIST)
        # Clamp cosTheta3 to [-1, 1] to avoid math domain errors
        cosTheta3 = max(-1, min(1, cosTheta3))
        theta3 = math.acos(cosTheta3)
        return -math.degrees(theta3), -theta3  # Negative theta3 value for downward forearm movement

    # Function to calculate the shoulder angle
    def shoulder_Theta(theta3_R):
        cosTheta2 = ((BICEP_DIST + FOREARM_DIST * math.cos(theta3_R)) * r2 + (FOREARM_DIST * math.sin(theta3_R)) * z2) / (math.pow(r2, 2) + math.pow(z2, 2))
        sinTheta2 = ((BICEP_DIST + FOREARM_DIST * math.cos(theta3_R)) * z2 - (FOREARM_DIST * math.sin(theta3_R)) * r2) / (math.pow(r2, 2) + math.pow(z2, 2))

        # Clamp cosTheta2 and sinTheta2 to [-1, 1] for safety
        cosTheta2 = max(-1, min(1, cosTheta2))
        sinTheta2 = max(-1, min(1, sinTheta2))

        theta2 = math.atan2(sinTheta2, cosTheta2)
        if theta2 < 0:  # Add offset when theta2 is negative
            theta2 += math.pi
        return math.degrees(theta2), theta2

    baseTheta, baseTheta_R = base_Theta()
    Theta3, Theta3_R = elbow_Theta()
    elbowTheta = Theta3 - 11  # Offset for elbow angle
    Theta2, Theta2_R = shoulder_Theta(Theta3_R)
    shoulderTheta = Theta2 + 11  # Offset for shoulder angle
    wristTheta = phi - elbowTheta - shoulderTheta  # Calculate wrist angle

    # Check for out-of-range conditions
    if baseTheta < BASE_L_LIMIT or baseTheta > BASE_H_LIMIT:
        print(f"Base angle ({baseTheta}) out of range [-135, 135]")
        return 0
    if shoulderTheta < SHOULDER_L_LIMIT or shoulderTheta > SHOULDER_H_LIMIT:
        print(f"Shoulder angle ({shoulderTheta}) out of range [0, 180]")
        return 0
    if elbowTheta > ELBOW_H_LIMIT or elbowTheta < ELBOW_L_LIMIT:
        print(f"Elbow angle ({elbowTheta}) out of range [-180, 0]")
        return 0
    if wristTheta > WRIST_H_LIMIT or wristTheta < WRIST_L_LIMIT:
        print(f"Wrist angle ({wristTheta}) out of range [-90, 90]")
        return 0

    print(f"Angles: [{int(baseTheta)}, {int(shoulderTheta)}, {int(elbowTheta)}, {int(wristTheta)}]")
    return [int(baseTheta + BASE_OFFSET), int(shoulderTheta + SHOUDLER_OFFSET), int(elbowTheta + ELBOW_OFFSET + ELBOW_W_OFFSET), int(wristTheta + WRIST_OFFSET)]
